<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('แก้ไขใบแจ้งหนี้') }} - {{ $invoice->document_number }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('invoices.update', $invoice) }}" id="invoiceForm">
                        ...
                        <div class="grid md:grid-cols-2 gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full" required>
                                    <option value="draft" {{ old('status', $invoice->status) == 'draft' ? 'selected' : '' }}>ฉบับร่าง</option>
                                    <option value="sent" {{ old('status', $invoice->status) == 'sent' ? 'selected' : '' }}>ส่งแล้ว</option>
                                    <option value="partial" {{ old('status', $invoice->status) == 'partial' ? 'selected' : '' }}>ชำระบางส่วน</option>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="paid_amount" class="block mt-1 w-full" type="number" step="0.01" name="paid_amount" :value="old('paid_amount', $invoice->paid_amount)" />
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="notes" name="notes" rows="3" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full">{{ old('notes', $invoice->notes) }}</textarea>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="terms" name="terms" rows="3" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full">{{ old('terms', $invoice->terms) }}</textarea>
