<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('แก้ไขใบเสนอราคา') }} - {{ $quotation->document_number }}
        </h2>
    </x-slot>
                        ...
                        <div class="grid md:grid-cols-2 gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <select id="status" name="status" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full" required>
                                    <option value="draft" {{ old('status', $quotation->status) == 'draft' ? 'selected' : '' }}>ฉบับร่าง</option>
                                    <option value="sent" {{ old('status', $quotation->status) == 'sent' ? 'selected' : '' }}>ส่งแล้ว</option>
                                    <option value="accepted" {{ old('status', $quotation->status) == 'accepted' ? 'selected' : '' }}>ยอมรับ</option>
                                    <option value="rejected" {{ old('status', $quotation->status) == 'rejected' ? 'selected' : '' }}>ปฏิเสธ</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="valid_until" class="block mt-1 w-full" type="date" name="valid_until" :value="old('valid_until', $quotation->valid_until->format('Y-m-d'))" required />
